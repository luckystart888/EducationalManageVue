<template>
    <div class="course">
        ...
        <el-form label-width="90px" ref="addFormRef" :model="addForm" :rules="addFormRules">
            ...
            <el-form-item label="教师工号" prop="teacherid" style="width: 60%">
                <el-input v-model.number="addForm.teacherid"></el-input>
            </el-form-item>
            <el-form-item label="上课日期" prop="courseweek" style="width: 40%">
            ...
            <el-form-item label="班级" prop="classid">
                <el-select v-model="addForm.classid" placeholder="班级" style="width: 160px">
                    <el-option
                    v-for="item in ClassName"
                    :key="item.id"
                    :label="item.name"
        ...
        <el-dialog title="修改课程" @close="aditClosed" :visible.sync="editDialogVisble" width="35%">
            <el-form :model="editForm" :rules="addFormRules" ref="editFormRef" label-width="90px">
                ...
                <el-form-item label="教师工号" prop="teacherid" style="width: 60%">
                    <el-input v-model.number="editForm.teacherid"></el-input>
                </el-form-item>
                <el-form-item label="上课日期" prop="courseweek" style="width: 40%">
                ...
                <el-form-item label="班级" prop="classid">
                    <el-select v-model="editForm.classid" placeholder="班级" style="width: 160px">
                        <el-option
                        v-for="item in ClassName"
                        :key="item.id"
                        :label="item.name"
...
export default {
  ...
  methods: {
    ...
    async getCourseList () {
      const { data: res } = await this.$http.get('admin/course/' + this.currentPage + '/' + this.pageSize, {
    ...
    addCourse () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 可以发起添加用户请求
        const { data: res } = await this.$http.post('admin/course', this.addForm)
        if (res.code !== 20011) {
          if (res.code === 60002) {
    ...
    addCourseTable () {
      this.$refs.addTableFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 可以发起添加用户请求
        const { data: res } = await this.$http.post('admin/courseTable', this.addTableForm)
        if (res.code !== 20011) {
          return this.$message.error('添加失败')
        }else{
    ...
    async showAddTableDialog (courseid) {
      const { data: res } = await this.$http.get('admin/courseTab/' + courseid)
      if (res.code !== 20041) {
    ...
    async showEditDialog (courseid) {
      const { data: res } = await this.$http.get('admin/course/' + courseid)
      if (res.code !== 20041) {
        return this.$message.error('查询用户数据失败~')
      }
    ...
    editCourseInfo () {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put('admin/course', this.editForm)
        if (res.code !== 20031) {
    ...
    async removeCourseById (courseid) {
      ...
      this.$http.delete('admin/course/' + courseid).then(res => {
