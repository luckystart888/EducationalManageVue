...
export default {
  ...
  methods: {
    ...
    async getCourseList () {
      const { data: res } = await this.$http.get('teacher/courseTable/' + this.currentPage + '/' + this.pageSize, {
    ...
    async showEditDialog (courseid) {
      const { data: res } = await this.$http.get('teacher/courseTable/' + courseid)
      if (res.code !== 20041) {
    ...
    editCourseInfo () {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put('teacher/courseTable', this.editForm)
        if (res.code !== 20031) {
    ...
    async removeCourseById (courseid) {
      ...
      this.$http.delete('teacher/courseTable/' + courseid).then(res => {
