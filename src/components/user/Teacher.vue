<template>
    <div class="users">
    <Breadcrumb name1="用户管理" name2="教师管理" />
    <!-- 卡片视图区域 -->
    <el-card class="box-card">
      <el-row :gutter="20">
        <el-col :span="16">
        <!-- 搜索与添加区域 -->
            <el-form :inline="true" :model="formTea">
                <el-form-item label="姓名:">
                    <el-input v-model="formTea.name" placeholder="姓名" style="width: 160px"></el-input>
                </el-form-item>
                <el-form-item label="性别:" prop="gender">
                    <el-select v-model="formTea.gender" placeholder="性别" style="width: 160px">
                        <el-option
                ...
                <el-form-item label="学院:" prop="departmentName">
                    <el-select v-model="formTea.departmentName" placeholder="所属学院" style="width: 160px">
                        <el-option
    ...
    <el-dialog title="添加用户" :visible.sync="addDialogVisible" width="35%" @close="addDislogClosed">
      ...
      <el-form label-width="70px" ref="addFormRef" :model="addForm" :rules="addFormRules">
        ...
        <el-form-item label="学院" prop="departmentName">
            <el-select v-model="addForm.departmentName" placeholder="所属学院">
                <el-option
                v-for="item in DepartmentName"
                :key="item.id"
                :label="item.departmentName"
        ...
        <el-form-item label="职称" prop="title">
            <el-select v-model="addForm.title" placeholder="职称">
                <el-option
                v-for="item in Title"
                :key="item.value"
                :label="item.label"
    ...
    <el-dialog title="修改用户" @close="aditClosed" :visible.sync="editDialogVisble" width="35%">
      <el-form :model="editForm" :rules="addFormRules" ref="editFormRef" label-width="70px">
        ...
        <el-form-item label="学院" prop="departmentName">
            <el-select v-model="editForm.departmentName" placeholder="所属学院">
                <el-option
                v-for="item in DepartmentName"
                :key="item.id"
                :label="item.departmentName"
        ...
        <el-form-item label="职称" prop="title">
            <el-select v-model="editForm.title" placeholder="职称">
                <el-option
                v-for="item in Title"
                :key="item.value"
                :label="item.label"
...
export default {
  ...
  data () {
    return {
      ...
      Title: [{ label: '教师', value: 'jiaoshi' }, { label: '副教授', value: 'fujiaoshou' }, { label: '教授', value: 'jiaoshou' }],
      // 存放用户的数据和数量
      userData: {
        userList: [],
        total: 0
  ...
  methods: {
    async getDepartmentName () {
      const { data: res } = await this.$http.get('/util/department')
      this.DepartmentName = res.data
      // console.log(this.DepartmentName)
    },
    async getUserList () {
      const { data: res } = await this.$http.get('/admin/info/teacher/' + this.currentPage + '/' + this.pageSize, {
        params: this.formTea
      })
      if (res.code === 20041) {
      ...
      this.formTea = { name: '', gender: '', departmentName: '' }
      // console.log(res)
    },
    ...
    addUser () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 可以发起添加用户请求
        const { data: res } = await this.$http.post('admin/info/teacher', this.addForm)
        if (res.code !== 20011) {
    ...
    async showEditDialog (userid) {
      const { data: res } = await this.$http.get('admin/info/teacher/' + userid)
      if (res.code !== 20041) {
    ...
    editUserInfo () {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put('admin/info/teacher', this.editForm)
        if (res.code !== 20031) {
    ...
    async removeUserById (userid) {
      ...
      this.$http.delete('admin/info/teacher/' + userid).then(res => {
